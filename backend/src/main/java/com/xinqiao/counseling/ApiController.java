...
public class ApiController {
    ...
    Map<String, Object> login(@RequestBody LoginRequest request) {
        ...
        // 审计日志：记录登录操作
        auditLog.log(authForLogin(request.username()), "LOGIN", "user",
            ((Number) user.get("id")).longValue(),
            (String) user.get("display_name") + "登录系统",
            null, null, null);
    ...
    Map<String, Object> updateProfile(Authentication auth, @RequestBody ProfileUpdateRequest request) {
        ...
        auditLog.log(auth, "UPDATE_PROFILE", "user_profile",
            ((Number) user.get("id")).longValue(),
            (String) user.get("display_name") + "更新了个人资料",
            null, null, null);
        return userProfile(user);
    }
    ...
    Map<String, Object> createAppointment(Authentication auth, @RequestBody AppointmentCreateRequest request) {
        ...
        var patient = jdbc.queryForMap("SELECT id, name FROM patient WHERE id = ?", patientId);
        ...
        jdbc.update(connection -> {
            ...
            ps.setLong(2, request.doctorId());
            ps.setString(3, request.topic().trim());
            ps.setString(4, request.appointmentTime().replace("T", " ") + ":00");
            ps.setString(5, isBlank(request.location()) ? "心桥心理咨询室" : request.location().trim());
            return ps;
        }, keyHolder);
        ...
        String newValue = """
            ...
            doctor.get("name"),
            request.topic().trim(),
            request.appointmentTime().replace("T", " "),
            isBlank(request.location()) ? "心桥心理咨询室" : request.location().trim()
        ).trim();
    ...
    List<Map<String, Object>> patientHistory(@PathVariable long patientId, Authentication auth) {
        ...
        var user = currentUser(auth);
        var patient = jdbc.queryForMap("SELECT name, primary_topic FROM patient WHERE id = ?", patientId);
        // 审计日志：记录查看敏感档案操作
        auditLog.log(auth, "VIEW_SENSITIVE", "patient", patientId,
            "查看" + patient.get("name") + "的咨询历史",
            null, null, "查阅既往咨询记录");
        return jdbc.queryForList("""
    ...
    List<Map<String, Object>> auditLogs(
        ...
        return auditLog.queryLogs(operationType, targetType, Math.min(limit, 500), offset);
    }
    ...
    Map<String, Object> auditLogsCount(
            @RequestParam(defaultValue = "") String operationType,
            @RequestParam(defaultValue = "") String targetType) {
        int total = auditLog.countLogs(operationType, targetType);
    ...
    private Authentication authForLogin(String username) {
        // 创建最小化的认证对象用于审计日志记录
